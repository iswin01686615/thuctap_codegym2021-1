...
<body>
    ...
    <form method="POST">
        <h1 style="text-align: center; color: #99080A;">Doi Mat Khau</h1>
    
        <table border="0" celspacing="0" cellpadding="0" id="table">
            <tr>
                <td align="center">Dia chi email(*)</td>
                <td align="center"><input type="email" name="acc" placeholder="Nhap email"></td>
            </tr>
            <tr>
                <td align="center">Mat khau cu(*)</td>
                <td align="center"><input type="password" name="oldpw" placeholder="Nhap mat khau cu"></td>
            </tr>
            <tr>
                <td align="center">Mat khau moi(*)</td>
                <td align="center"><input type="password" name="newpw" placeholder="Nhap mat khau moi"></td>
            </tr>
            <tr>
                <td align="center">Nhap lai mat khau moi(*)</td>
                <td align="center"><input type="password" name="re-newpw" placeholder="Nhap lai mat khau moi"></td>
            </tr>
            <tr>
                <td colspan="2" align="center">
                <input class="btn btn-primary" type="submit" name="ok" value="Change Password">
               </td>
            </tr>
        </table>
    </form>
...
<?php
    ...
    if(isset($_POST['ok']))
    {
        if( empty($_POST['acc']) || empty($_POST['oldpw']) || empty($_POST['newpw']) || empty($_POST['re-newpw']))
          echo "<script> alert('Ban chua nhap du lieu')</script>";
                elseif ($_POST['newpw'] != $_POST['re-newpw'])
                    echo "<script> alert('Mat khau khong trung nhau. Nhap lai!')</script>";
                elseif($_POST['oldpw'] != $_POST['newpw'])
                {

                    $user = new Account();
                    $slpw = $user->select_pw($_POST['acc']);
                    foreach($slpw as $sl)
                    {
                        if($_POST['oldpw'] == $sl['Password'])
                        {
                            $add_user=$user->Update($_POST['acc'],$_POST['newpw']);
                            if($add_user)
                            {
                                echo "<script> alert('Doi mat khau thanh cong')</script>";
                                echo "<script> location.href='acc.php'</script>";
                            }
                ...
                else echo "<script> alert('Mat khau cu va mat khau moi phai khac nhau')</script>";
